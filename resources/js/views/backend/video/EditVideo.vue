<template>
    <div class="w-full bg-white rounded-xl overflow-hidden shadow">
        ...
        <div class="w-full p-5 bg-gray-100 flex items-center justify-between">
            <h4 class="text-xl">ویرایش ویدیو</h4>
        ...
        <div v-if="store.state.videos.loading" class="text-center">
            <span>در حال بارگزاری ویدیو</span>
        </div>
        <!-- card body:start -->
        <div class="p-5 bg-white" v-else>
                ...
                <div class="flex items-center justify-between gap-5">
                    <div class="flex-1">
                        ...
                        <button type="button" class="w-full rounded-xl bg-gray-100 relative outline-none px-3 py-2 my-3 overflow-hidden">
                            <input @change="selectPoster" type="file" name="" id="" class="absolute top-0 left-0 w-full h-full opacity-0">
                        ...
                        <div class="m-3">
                            <p class="mb-3">پستر فعلی</p>
                            <div class="w-full h-32 rounded-xl bg-gray-100 flex items-center justify-center text-3xl overflow-hidden">
                                <img v-if="currentData.poster" :src="currentData.poster" alt="" class="w-full object-cover">
                                <ion-icon v-else name="image-outline"></ion-icon>
                            </div>
                        </div>
                    ...
                    <div class="flex-1">
                        ...
                        <button type="button" class="w-full rounded-xl bg-gray-100 relative outline-none px-3 py-2 my-3 overflow-hidden">
                            <input @change="selectTeaser" type="file" name="" id="" class="absolute top-0 left-0 w-full h-full opacity-0">
                            <div class="flex items-center gap-3 w-full justify-center">
                        ...
                        <div class="m-3">
                            <p class="mb-2">تیزر فعلی</p>
                            <div class="w-full h-32 rounded-xl bg-gray-100 flex items-center justify-center text-3xl overflow-hidden">
                                <video v-if="currentData.teaser" width="400" controls>
                                    <source :src="currentData.teaser" type="video/mp4">
                                    <source :src="currentData.teaser" type="video/mvc">
                                    <source :src="currentData.teaser" type="video/ogg">
                                    مرور گر شما نمیتواند این ویدیو رو ساپورت کند
                                </video>
                    ...
                    <div class="flex-1">
                        ...
                        <button type="button" class="w-full rounded-xl bg-gray-100 relative outline-none px-3 py-2 my-3 overflow-hidden">
                            <input @change="selectVideo" type="file" name="" id="" class="absolute top-0 left-0 w-full h-full opacity-0">
                            <div class="flex items-center gap-3 w-full justify-center">
                        ...
                        <div class="m-3">
                            <p class="mb-2">فیلم فعلی</p>
                            <div class="w-full h-32 rounded-xl bg-gray-100 flex items-center justify-center text-3xl overflow-hidden">
                                <video v-if="currentData.video" width="400" controls>
                                    <source :src="currentData.video" type="video/mp4">
                                    <source :src="currentData.video" type="video/mvc">
                                    <source :src="currentData.video" type="video/ogg">
                                    مرور گر شما نمیتواند این ویدیو رو ساپورت کند
                                </video>
                                <ion-icon v-else name="videocam-outline"></ion-icon>
                            </div>
            ...
            <div class="my-3">
                <div class="flex md:flex-row flex-col items-center justify-between gap-5">
                    <div class="md:flex-1 w-full">
                        ...
                        <button type="button" class="w-full rounded-xl bg-gray-100 relative outline-none px-3 py-2 my-3 overflow-hidden">
                            <input @change="selectJustification" type="file" name="" id="" class="absolute top-0 left-0 w-full h-full opacity-0" accept=".docx,.pdf,.png,.jpg,.jpeg">
                            <div class="flex items-center gap-3 w-full justify-center">
                                <ion-icon name="videocam-outline"></ion-icon>
            ...
            <div class="my-3">
                <button @click="saveVideo" class="bg-yellow-400 mt-3 px-3 py-2 text-white rounded-xl duration-150 hover:bg-yellow-500">ویرایش فیلم</button>
            </div>
...
<script setup>
    ...
    // load the video
    onMounted(() => {
        store.dispatch('getCurrentVideo', {id : route.params.id})
            .then(() => {
                videoData.value = store.state.videos.currentVideo;
                currentData.poster = videoData.value.poster;
                currentData.video = videoData.value.video;
                currentData.teaser = videoData.value.teaser;
                videoData.value.video = '';
                videoData.value.poster = '';
                videoData.value.teaser = '';
            })
    })
    ...
    let saveVideo = () => {
        ...
        store.dispatch('editVideo', {data: videoData.value, id: route.params.id})
            .then(() => {
